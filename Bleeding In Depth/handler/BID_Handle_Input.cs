...
using static BleedingInDepth.handler.BID_Handle_Entity;

namespace BleedingInDepth.handler
{
    internal class BID_Handle_Input
    {
        //private static ICoreAPI API = BID_VarRef.API;
        private static ICoreClientAPI clientAPI = BID_VarRef.API_Client;
        ...
        internal class Handle_Command
        {
            ...
            internal static TextCommandResult Command_MakeBleed(TextCommandCallingArgs args)//TODO: switch to lang
            {
                ...
                entity_BleedBehavior.State_BleedReductionFlag = BID_Function_General.Calc_Flag_SetBit(entity_BleedBehavior.State_BleedReductionFlag, 3, false); entity_BleedBehavior.State_BleedReductionFlag = BID_Function_General.Calc_Flag_SetBit(entity_BleedBehavior.State_BleedReductionFlag, 2, false);
        ...
        internal static bool Handle_Hotkey(KeyCombination playerInput, string keyCaller) //called on all hotkeys to handle I/O
        {
            switch (keyCaller)
            {
                ...
                default: { BID_Function_General.Log_Error("Hotkey key passed is invalid: {0}", loggers: [keyCaller]); return false; }
            }
        }
            ...
            internal static bool Handle_ReportBleed(Entity caller, bool? skipCheckEntity)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        float DPS_External = entity_BleedBehavior.Bleed_CurrentLevel_External * Config_Reference.Config_Loaded.Curve_Variable.Variable_External_Acc.External_Rate;
                        float DPS_Internal = entity_BleedBehavior.Bleed_CurrentLevel_Internal * Config_Reference.Config_Loaded.Curve_Variable.Variable_Internal_Acc.Internal_Rate * BID_VarRef.normalizer_InternalBleed;
                    ...
                    reportOutput_BleedReduction = (BID_Function_General.Calc_Flag_CheckBit(entity_BleedBehavior.State_BleedReductionFlag, 3) ? " and has bandaged" : (BID_Function_General.Calc_Flag_CheckBit(entity_BleedBehavior.State_BleedReductionFlag, 2) ? " and has ragged" : ""));
                    ...
                    BID_Function_General.Log_Debug("Player recieved bleed report for External: {0}; Internal: {1}; State_BleedReduction: {2}", loggers: [entity_BleedBehavior.Bleed_CurrentLevel_External, entity_BleedBehavior.Bleed_CurrentLevel_Internal, entity_BleedBehavior.State_BleedReductionFlag]);
                    return true;
                }
            }
        }
