...
{
    internal class BID_Manager_Collection 
    {
        private static ICoreAPI API = BID_VarRef.API;

        internal static void Dicitionary_Freeze()
        {
            BID_Function_Bleed.DamageType_Dict_ConfigCache = Config_Reference.Config_Loaded.Config_TypeModifier.TypeMod_Damage_Acc.Dict_DamageType.ToFrozenDictionary();
        }
        ...
        internal class HandleDictionary() //this is probably unneeded now as the config itself is building the dictionary; leaving here in case i can use it again
        {
            ...
            internal static bool Dict_BuildVerify(string RequestedDictName) //invalid check -> attempt to rebuild -> return build success (t/f)
            {
                try
                {
                    ...
                    {
                        case null: { BID_Function_General.Log_Error("Recieved null RequestedDictName: fallback to vanilla damage system"); return false; }
                        ...
                        default: { BID_Function_General.Log_Debug("Dict verificaiton failed, Dict type does not exist: {0}", loggers: [RequestedDictName]); return false; }
                    }
                }
                catch { BID_Function_General.Log_Error("Dict verificaiton failed: {0}", loggers: [RequestedDictName]); return false; }
            }
            ...
            {
                if (Config_Reference.Config_Loaded == null) { BID_Function_General.Log_Error("Config file is missing. Falling back to vanilla damage system."); return []; }
                BID_Function_General.Log_Debug("Building dict: {0}", loggers: [RequestedDictName]);
                switch (RequestedDictName) //TODO: convert to for loops
                {
                    case null: { BID_Function_General.Log_Error("Recived null RequestedDictName, fallback to vanilla damage system"); return []; }
                    ...
                    default: { BID_Function_General.Log_Debug("Failed to build dicitonary: ({0})", loggers: [RequestedDictName]); Dict_DamageTypeValues = []; return []; }
                }
            }
        }
