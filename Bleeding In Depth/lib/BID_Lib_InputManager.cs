    ...
    internal class BID_Lib_InputManager
    {
        ...
        internal class Handle_Command
        {
            ...
            internal static TextCommandResult Command_ReportBleed(TextCommandCallingArgs args)
            {
                ...
                Handle_UserInputs.Handle_ReportBleed(caller, (args[0] is not true) ? false : true);
        ...
        internal static bool Handle_Hotkey(KeyCombination playerInput, string keyCaller) //called on all hotkeys to handle I/O
        {
            switch (keyCaller)
            {
                ...
                default: { BID_Lib_FunctionsGeneral.Log_Debug("Hotkey key passed is invalid: {0}", loggers: [keyCaller]); return false; }
            }
        }
            ...
            internal static bool Handle_ReportBleed(Entity caller, bool? skipCheckEntity)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        float DPS_External = entity_BleedBehavior.Bleed_CurrentLevel_External * Config_Reference.Config_Loaded.Config_Rate.Rate_BleedDamage_External;
                        float DPS_Internal = entity_BleedBehavior.Bleed_CurrentLevel_Internal * Config_Reference.Config_Loaded.Config_Rate.Rate_BleedDamage_Internal * BID_Lib_FunctionsBleed.normalizer_InternalBleed;
                    ...
                    clientAPI.Event.EnqueueMainThreadTask(() => { clientAPI.ShowChatMessage(reportOutput_External + reportOutput_Internal); }, "");
                    BID_Lib_FunctionsGeneral.Log_Debug("Player recieved bleed report for External: {0}; Internal: {1}", loggers: [entity_BleedBehavior.Bleed_CurrentLevel_External, entity_BleedBehavior.Bleed_CurrentLevel_Internal]);
                    return true;
                }
            }
        }
