...
{
    internal class BID_Function_Bleed
    {
        ...
        internal static float BleedDamage_Conversion_Store(Entity entity, float appliedDamage, ref DamageSource damageSource)
        {
            ...
            if (appliedDamage <= 0f) { BID_Function_General.Log_Debug("appliedDamage was not positive! {0}", loggers: [appliedDamage]); return BID_VarRef.CancelAndReturnValue; }
            if (entity.GetBehavior<EntityBehaviorHealth>() is not EntityBehaviorHealth entity_BehaviorHealth || entity.GetBehavior<BID_Manager_Entity.EntityBehavior_Bleed>() is not BID_Manager_Entity.EntityBehavior_Bleed entity_BehaviorBleed) { BID_Function_General.Log_Debug("Entity was missing BehaviorHealth or BehaviorBleed: {0}", loggers: [entity.GetName() ?? entity.GetPrefixAndCreatureName() ?? "null entity"]); return BID_VarRef.CancelAndReturnValue; }
            ...
            switch (damageSource.Type)
            {
                ...
                case EnumDamageType.Fire: { BleedDamage_Cauterize(entity, appliedDamage); return BID_VarRef.CancelAndReturnValue; } //TODO: see if i can inject in entity.ApplyFireDamage or similar method instead
            }
            ...
            if (DamageType_Dict_ConfigCache?.TryGetValue(damageSource.Type, out var DamageType_OuterDict) is not true) { BID_Function_General.Log_Debug("Dict_DamageType was null or missing damageType: {0}", loggers: [damageSource.Type]); return BID_VarRef.CancelAndReturnValue; }
            if (DamageType_OuterDict?.TryGetValue(BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_DirectMulti, out var _) is not true || DamageType_OuterDict?.TryGetValue(BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedMultiExternal, out var _) is not true) { BID_Function_General.Log_Debug("Dict_DamageType does not contain innerdict value(s) within Dict_DamageType: {0}", loggers: [damageSource.Type]); return BID_VarRef.CancelAndReturnValue; } //dont check for internal bleed mods as DamageTypes can not have internal bleeding. Instead skip applying internal if missing

            //store entity category type
            if (entity_BehaviorBleed.categoryType_Dict is null)
            {
                ...
                if (Config_Reference.Config_Loaded.Config_TypeModifier?.TypeMod_Entity_Acc?.Dict_EntityCategory?.TryGetValue(entity_CategoryType, out Dictionary<string, float>? entityCategoryDict) is not true)
                {
                    ...
                    entityCategoryDict = new() { [BID_Config_Main.Config_TypeModifier.TypeMod_Entity.NameOf_EntityCategory_DamageMod_Direct] = 1f, [BID_Config_Main.Config_TypeModifier.TypeMod_Entity.NameOf_EntityCategory_DamageMod_Bleed_External] = 1f, [BID_Config_Main.Config_TypeModifier.TypeMod_Entity.NameOf_EntityCategory_DamageMod_Bleed_Internal] = 1f };
                }
            ...
            entity_BehaviorBleed.health_PreDamage = entity_BehaviorHealth.Health;
        ...
        internal static void BleedDamage_Conversion_Apply(Entity entity, DamageSource lastBleedSource)
        {
            ...
            if (entity.GetBehavior<EntityBehaviorHealth>() is not EntityBehaviorHealth entity_BehaviorHealth || entity.GetBehavior<BID_Manager_Entity.EntityBehavior_Bleed>() is not BID_Manager_Entity.EntityBehavior_Bleed entity_BehaviorBleed) { BID_Function_General.Log_Debug("Entity was missing BehaviorHealth or BehaviorBleed: {0}", loggers: [entity.GetName() ?? entity.GetPrefixAndCreatureName() ?? "null entity"]); return; }
            if (Config_Reference.Config_Loaded.Config_TypeModifier?.TypeMod_Damage_Acc?.Dict_DamageType?.TryGetValue(lastBleedSource.Type, out var DamageType_OuterDict) is not true) { BID_Function_General.Log_Debug("Dict_DamageType was null or missing damageType: {0}", loggers: [lastBleedSource.Type]); return; }//redeclare dict in case another entity calls bleed conversion in the same tick
            if (DamageType_OuterDict?.TryGetValue(BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_DirectMulti, out var _) is not true || DamageType_OuterDict?.TryGetValue(BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedMultiExternal, out var _) is not true) { BID_Function_General.Log_Debug("Dict_DamageType does not contain innerdict value(s) within Dict_DamageType: {0}", loggers: [lastBleedSource.Type]); return; } //dont check for internal bleed mods as DamageTypes can not have internal bleeding. Instead skip applying internal if missing
            ...
            {
                //external bleed
                if (Config_Reference.Config_Loaded.Config_System.System_Damage_Acc.Bleed_External)
                {
                    ...
                    bleedToApply_External = MathF.Round(MathF.Max(damage_ToConvert * DamageType_OuterDict[BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedMultiExternal], 0.0f), 4);
                }
                else { BID_Function_General.Log_Debug("Bleed_External is disabled in config"); return; }
                ...
                if (Config_Reference.Config_Loaded.Config_System.System_Damage_Acc.Bleed_Internal)
                {
                    ...
                    {
                        if (damage_ToConvert > DamageType_OuterDict[BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedConversionThresholdInternal] && DamageType_OuterDict[BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedMultiInternal] != 0)
                        {
                        ...
                        if (DamageType_OuterDict[BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedMultiInternal] == 0) //if [Bleed_Multi_Internal] is 0 all external bleed applied is converted directly into internal 
                        {
                            bleedToApply_Internal = MathF.Round(MathF.Max(damage_ToConvert * DamageType_OuterDict[BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedMultiExternal] - DamageType_OuterDict[BID_Config_Main.Config_TypeModifier.TypeMod_Damage.NameOf_DamageType_BleedConversionThresholdInternal], 0.0f), 4);
                    ...
                    else { BID_Function_General.Log_Debug_Verbose("Dict_DamageType does not contain internal bleed values and is invalid for DamageType: {0}", loggers: [lastBleedSource.Type]); }
                }
                else { BID_Function_General.Log_Debug_Verbose("Bleed_Internal is disabled in config"); return; }
            }
            catch(Exception e) { BID_Function_General.Log_Error("Exception caught: {0}", loggers: [e.Message]); return; }
            ...
            entity_BehaviorBleed.Bleed_CurrentLevel_External += bleedToApply_External * entity_BehaviorBleed.categoryType_Dict[BID_Config_Main.Config_TypeModifier.TypeMod_Entity.NameOf_EntityCategory_DamageMod_Bleed_External];
            entity_BehaviorBleed.Bleed_CurrentLevel_Internal += bleedToApply_Internal * entity_BehaviorBleed.categoryType_Dict[BID_Config_Main.Config_TypeModifier.TypeMod_Entity.NameOf_EntityCategory_DamageMod_Bleed_Internal];
            ...
            BID_Function_General.Log_Debug("Applied {0} damage, {1} Bleed_External and {2} Bleed_Internal to {3} from {4} which dealt {5} base damage of {6} DamageType",
                loggers: [entity_BehaviorBleed.health_PreDamage - entity_BehaviorHealth.Health, bleedToApply_External, bleedToApply_Internal, entity.GetName() ?? entity.GetPrefixAndCreatureName(), lastBleedSource.CauseEntity?.GetPrefixAndCreatureName() ?? lastBleedSource.SourceEntity?.GetPrefixAndCreatureName() ?? "null", entity_BehaviorBleed.appliedDamage_Base, lastBleedSource.Type]);
        ...
        {
            if (entity.GetBehavior<BID_Manager_Entity.EntityBehavior_Bleed>() is not BID_Manager_Entity.EntityBehavior_Bleed entity_BehaviorBleed) { return; } if (entity.GetBehavior<EntityBehaviorHealth>() is not EntityBehaviorHealth entity_BehaviorHealth) { return; }
            ...
            if (entityPlayer is not null)
            {
                ...
                if (Config_Reference.Config_Loaded.Config_System.System_Damage_Acc.Bleed_SlowAtLowHealth) { deltaTimeSum *= BID_Function_General.Calc_Curve_ExpoEaseOut(entity_BehaviorHealth.Health, 0.8f, 0.8f, 0f, 0.2f); } //TODO: expose these in config
                ...
                entity_BehaviorBleed.State_BleedReductionFlag = BID_Function_General.Calc_Flag_SetBit(entity_BehaviorBleed.State_BleedReductionFlag, 1, (isResting ? true : false));
            ...
            {
                if (false) { entity_BehaviorBleed.State_BleedReductionFlag = BID_Function_General.Calc_Flag_SetBit(entity_BehaviorBleed.State_BleedReductionFlag, 0, true); } else { entity_BehaviorBleed.State_BleedReductionFlag = BID_Function_General.Calc_Flag_SetBit(entity_BehaviorBleed.State_BleedReductionFlag, 0, false); }
                state_HealBonus_Resting = false ? Config_Reference.Config_Loaded.Curve_Variable.Variable_HealBonus_Acc.HealBonus_BleedReduction_Care : 0f;
            }
            ...
            {
                float damageToAdd = entity_BehaviorBleed.Bleed_CurrentLevel_External * (deltaTimeSum * Config_Reference.Config_Loaded.Curve_Variable.Variable_External_Acc.External_Rate / MathF.Max((state_BleedReduction_PressureOrCare + state_BleedReduction_BandagedOrRagged), 1f));
                ...
                entity_BehaviorBleed.Bleed_CurrentLevel_External -= (deltaTimeSum * Config_Reference.Config_Loaded.Curve_Variable.Variable_External_Acc.External_FlatHeal * MathF.Max(state_HealBonus_Resting, 1f)) + (deltaTimeSum * entity_BehaviorBleed.Bleed_CurrentLevel_External * Config_Reference.Config_Loaded.Curve_Variable.Variable_External_Acc.External_ScaledHeal);
            }

            //calc internal bleed damage
            if (entity_BehaviorBleed.Bleed_CurrentLevel_Internal > 0f)
            {
                float damageToAdd = entity_BehaviorBleed.Bleed_CurrentLevel_Internal * (deltaTimeSum * Config_Reference.Config_Loaded.Curve_Variable.Variable_Internal_Acc.Internal_Rate / MathF.Max( 0f/*TODO: add limited internal stemming check*/, 1f));
                ...
                entity_BehaviorBleed.Bleed_CurrentLevel_Internal -= (deltaTimeSum * Config_Reference.Config_Loaded.Curve_Variable.Variable_Internal_Acc.Internal_FlatHeal * MathF.Max(state_HealBonus_Resting * (entityPlayer?.Controls.FloorSitting is true ? Config_Reference.Config_Loaded.Curve_Variable.Variable_HealBonus_Acc.HealBonus_BleedHeal_InternalRestBonus : 1f), 1f)) + (deltaTimeSum * entity_BehaviorBleed.Bleed_CurrentLevel_Internal * Config_Reference.Config_Loaded.Curve_Variable.Variable_Internal_Acc.Internal_ScaledHeal);
            }
        ...
        {
            if (entity.GetBehavior<BID_Manager_Entity.EntityBehavior_Bleed>() is not BID_Manager_Entity.EntityBehavior_Bleed entity_BehaviorBleed) { return appliedDamage; }
            float bleed_ToRemove = MathF.Max(appliedDamage * Config_Reference.Config_Loaded.Curve_Variable.Variable_HealBonus_Acc.HealBonus_BleedReduction_Flat_Bandage, 0f);
            float heal_PreCalc = appliedDamage;

            if (appliedDamage >= Config_Reference.Config_Loaded.Curve_Variable.Variable_HealBonus_Acc.HealBonus_BleedHeal_RagToBandageThreshold) { entity_BehaviorBleed.State_BleedReductionFlag = BID_Function_General.Calc_Flag_SetBit(entity_BehaviorBleed.State_BleedReductionFlag, 3, true); } else { entity_BehaviorBleed.State_BleedReductionFlag = BID_Function_General.Calc_Flag_SetBit(entity_BehaviorBleed.State_BleedReductionFlag, 2, true); }//TODO: only apply this if the item used is certain healing items (ie. bandage)
